...
public class CommonUtils {
    ...
    public final static int DEFAULT_VIDEO_THUMB_SIZE = 150;
...
//        return snapshotFolder;
    ...
    public static Bitmap getVideoThumbnail(String filePath, int width, int height) {
        ...
        try {
            ...
            Bitmap resultBitmap = ThumbnailUtils.extractThumbnail(bitmap, width, height);
            return resultBitmap;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
...
//            cacheSize += getFolderSize(FileUtil.getBlurPhotoDir());
        }
    ...
    public static void clearAllCache(final Context context, final Runnable callback) {
        ThreadManager.execute(new Runnable() {
            ...
            public void run() {
                ...
                deleteCacheFile(FileUtil.getCacheDir());
                if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
    ...
    public static void deleteCacheFile(File cacheDir) {
        if (cacheDir != null && cacheDir.isDirectory()) {
            try {
                File[] children = cacheDir.listFiles();
                for (int i = 0; i < children.length; i++) {
                    FileUtil.deleteDir(children[i]);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }

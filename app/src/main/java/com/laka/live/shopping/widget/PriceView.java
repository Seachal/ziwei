...
public class PriceView extends View {
    ...
    private boolean bold = false; // 是否是粗体
    ...
    private void calcTextDimens() {
        ...
        //处理前缀
        int prefixWidth = TextUtils.isEmpty(prefix) ? 0 : process(prefix, intSize);
        prefixStart = getPaddingLeft();

        //处理￥
        int moneyWidth = process(money, moneySize);
        moneyStart = prefixStart + prefixWidth;
    ...
    public void draw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(prefix)) {
            canvas.drawText(prefix, 0, commonY, mPaint);
        }
        //画￥
    ...
    private int measureHeight(int measureSpec) {
        ...
        return result + mContext.getResources().getDimensionPixelSize(R.dimen.space_1);
    }
    ...
    private void getProperties(Context context, AttributeSet attrs) {
        ...
        int textSize = a.getDimensionPixelSize(R.styleable.CartPriceValue_CarttextSize, 14);
        String value = a.getString(R.styleable.CartPriceValue_Cartvalue);
        int textColor = a.getColor(R.styleable.CartPriceValue_CarttextColor, 0xffffff);
        ...
        this.bold = SystemUtil.getAttributeBooleanValue(attrs, R.attr.bold, false);
        this.prefix = a.getString(R.styleable.CartPriceValue_prefix);
        this.value = value;

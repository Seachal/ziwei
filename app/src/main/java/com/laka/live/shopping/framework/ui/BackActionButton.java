...
public class BackActionButton extends LinearLayout {
    ...
    private void initComponent() {
        ...
        mImageView.setImageDrawable(ResourceHelper.getDrawable(R.drawable.title_back_gray));
        mImageView.setScaleType(ImageView.ScaleType.CENTER);

        mTitleTextView = new TextView(getContext());
        mTitleTextView.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        mTitleTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ResourceHelper.getDimen(R.dimen.defaultwindow_title_text_size));
        int titleTextPadding = ResourceHelper.getDimen(R.dimen.titlebar_title_text_padding);
        mTitleTextView.setPadding(0, 0, titleTextPadding, 0);
        mTitleTextView.setGravity(Gravity.CENTER);
        mTitleTextView.setSingleLine();
        mTitleTextView.setEllipsize(TruncateAt.END);
        mTitleTextView.setVisibility(View.GONE);
        mTitleTextView.setTextColor(ResourceHelper.getColor(R.color.title_text_color));
    ...
    // 隐藏返回键ICON,同时要给文本设置padding，不然文本会直接靠左
    public void setBackActionIconVisibility(int visibility) {
        mImageView.setVisibility(visibility);
        mTitleTextView.setPadding(ResourceHelper.getDimen(R.dimen.space_10),0,ResourceHelper.getDimen(R.dimen.space_10),0);
    }

...
public class EmojiManager {
    ...
    static {
        final Context context = LiveApplication.getInstance().getApplicationContext();
...
//                    oldValue.recycle();
                AnalyticsReport.reportError(context , AnalyticsReport.BITMAP_LRUCACHE_ENTRYREMOVE_METHOD_CALLED);
                Log.e("test", "entryRemoved : " + "key");
            }
        };
    }
    ...
    //预加载表情图片
    public static void initDrawables(final Context context) {
        BackgroundThread.post(new Runnable() {
            @Override
            public void run() {
                for (String key : text2entry.keySet()) {
                    Entry entry = text2entry.get(key);
                    if (drawableCache.get(entry.assetPath) == null) {
                        loadAssetBitmap(context, entry.assetPath);
                    }
                }
            }
        });

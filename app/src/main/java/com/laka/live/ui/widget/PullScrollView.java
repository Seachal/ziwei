...
public class PullScrollView extends ScrollView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (null != attrs) {
            ...
            if (ta != null) {
                ...
                mMaxScrallHeight = (int) ta.getDimension(R.styleable
    ...
    public void setMaxScrallHeight(int height) {
        mMaxScrallHeight = height;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        try {
            if (ev.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                if (deltaY >= configuration.getScaledTouchSlop() && deltaY > Math.abs(ev.getX() - mStartPoint.x)) {
                    onTouchEvent(ev);
                    return true;
                }
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isMoving) {
            try {
                ...
                com.laka.live.util.Log.error("test", "isHandle : " + isHandle);
            } catch (Exception e) {
    ...
    private boolean isEnableHeadViewUpAnim = true;
    ...
    private void doActionMove(MotionEvent event) {
        ...
        if (mState == State.UP) {
            ...
            isMoving = false;
            isEnableHeadViewUpAnim = false;
        } else if (mState == State.DOWN) {
            isEnableHeadViewUpAnim = true;
            if (getScrollY() <= deltaY) {
                isMoving = true;
            }
            float maxDelTaY = mMaxScrallHeight / SCROLL_RATIO;
            deltaY = deltaY < 0 ? 0 : (deltaY > maxDelTaY ? maxDelTaY : deltaY);
        }
        ...
        if (isMoving) {
            ...
            int headerBottom = mCurrentBottom - (mHeaderHeight - mMaxScrallHeight);
    ...
    private void rollBackAnimation() {
        int distance = Math.abs(mInitTop - mCurrentTop);
//        if (!isEnableHeadViewUpAnim) {
//            distance = -distance;
//        }

...
public class PopupMenu {
    ...
    private static final float DEFAULT_TEXT_SIZE_PIXEL = ResourceHelper.getDimen(R.dimen.sp30);
    private static final int DEFAULT_NORMAL_BACKGROUND_COLOR = Color.BLACK;
    ...
    public void init(Activity context,
                     ...
                     OnPopupListClickListener onPopupListClickListener) {
     ...
     if (mAnchorView instanceof AbsListView) {
        ...
        refreshBackgroundOrRadiusStateList(mPressedBackgroundColor,
                mNormalBackgroundColor, mBackgroundCornerRadiusPixel);
        refreshTextColorStateList(mPressedTextColor, mNormalTextColor);
    }

    private void showPopupListWindow() {
        if (mContext instanceof Activity && mContext.isFinishing()) {
            return;
        }
        if (mPopupWindow == null) {
            ...
            for (int i = 0; i < mPopupItemList.size(); i++) {
                ...
                textView.setPadding(dp2px(10), dp2px(8), dp2px(10), dp2px(8));
                textView.setClickable(true);
                final int mMenuItemPosition = i;
                textView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (mOnPopupListClickListener != null) {
                            mOnPopupListClickListener
                                    .onPopupListClick(mPosition,mMenuItemPosition);
                            hidePopupListWindow();
                        }
                    }
                });
            ...
            if (mPopupWindowHeight == 0) {
                mPopupWindowHeight = getViewHeight(popupListContainer)
                        + mIndicatorHeight;
            }
        ...
        if (mAnchorView instanceof AbsListView) {
            ...
            mPopupWindow.showAtLocation(mAnchorView, Gravity.NO_GRAVITY,
                    location[0]-mPopupWindow.getWidth() / 2 + mAnchorView.getWidth()/2,
    ...
    public interface OnPopupListClickListener {
        void onPopupListClick(int mListItemPosition, int mMenuItemPosition);
    }

    public void notifyDataChange() {
        mPopupWindow = null;
        mPopupWindowWidth = 0;
        init(mContext,mAnchorView,mPopupItemList,mOnPopupListClickListener);

...
//        setTabbarContainerBg(new ColorDrawable(TitleBar.getBgColor()));
        ...
        mCursor.setCursorColor(ResourceHelper.getColor(R.color.colorFF950b));
        mTabbarDivider.setBackgroundColor(ResourceHelper.getColor(R.color.colorE5E5E5));
    }

    private void initTabbar(boolean bottomTabbar) {
        Context context = getContext();
        mTabbarContainer = new LinearLayout(context);
        mTabbarContainer.setId(ID_TABBAR_CONTAINER);
        mTabbarContainer.setOrientation(LinearLayout.VERTICAL);
        ...
        mTabbarDivider = new View(context);
        if (bottomTabbar) {
            LinearLayout.LayoutParams llp = new LinearLayout.LayoutParams(
                    LayoutParams.MATCH_PARENT, ResourceHelper.getDimen(R.dimen.divider_height));
            mTabbarContainer.addView(mTabbarDivider, llp);
        }
        ...
        mTabbarContainer.addView(mTabbar, llp);

        mCursor = new TabCursor(context);
        llp = new LinearLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, mCursorHeight);
//        mCursorWidth = Utils.dip2px(getContext(),15);
//        mCursorHeight = Utils.dip2px(getContext(),2);
//        llp = new LinearLayout.LayoutParams(
//                mCursorWidth, mCursorHeight);
        mTabbarContainer.addView(mCursor, llp);
        ...
        mTabbarDivider.setVisibility(View.GONE);
    }
    ...
    public void pullTabbarIn(FrameLayout tabBarLayout, int margin) {
        if (mTabbarContainer.getParent() != null) {
            ViewParent viewParent = mTabbarContainer.getParent();
            ((ViewGroup) viewParent).removeView(mTabbarContainer);
        }
        ...
        tabBarLayout.addView(mTabbarContainer, TAB_BAR_INDEX, lp);
    }

    public void initDefaultStyle() {
//        this.setTabbarContainerBg(new ColorDrawable(
//              ResourceHelper.getColor(R.color.white)));
    ...
    public void onClick(View v) {
        int tabIndex = v.getId() - TAB_ITEM_ID_START;
        if (mListener != null) {
            if (mListener.onTabChangPreProcess(tabIndex)) {
                return;
            }
        }
        snapToTab(tabIndex, mEnableSnapAnimOnTitleClick);
        if (mListener != null) {
    ...
    public void snapToTab(int index, boolean withAnimation) {
        if (mListener != null) {
            if (mListener.onTabChangPreProcess(index)) {
                return;
            }
        }
    ...
    public void showTabDivider() {
        mTabbarDivider.setVisibility(View.VISIBLE);
    }
    ...
    public void setCursorHeight(int height) {
        ...
        LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mCursor
                .getLayoutParams();
        lp.height = height;
    ...
    public static int convertViewId2Index(int viewId) {
        return viewId - TAB_ITEM_ID_START;
    }
}
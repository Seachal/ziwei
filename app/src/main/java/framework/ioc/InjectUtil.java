...
import framework.ioc.annotation.InjectAssert;
import framework.ioc.annotation.InjectExtra;
import framework.ioc.annotation.InjectResource;
...
public class InjectUtil {
    ...
    public static void inject(Object obj) {
        if (obj == null)
            return;
        // 本类中的所有属性
        Field[] fields = getPublicFields(obj.getClass());
    ...
    public static void injectView(Object obj, View view, boolean isInjectAllView) {

        if (isInjectAllView) {
            doInjectView(getDeclaredFields(obj.getClass()), obj, view);
        } else {
            injectView(obj, view);
        }
    ...
    public static void injectView(Object obj, View view) {
        // 本类中的所有属性
        Field[] fields = getPublicFields(obj.getClass());
        doInjectView(fields, obj, view);
    }

    public static void doInjectView(Field[] fields, Object obj, View view) {
                ...
                try {
                    ...
                    if (viewInject != null) {
                        ...
                        if (!TextUtils.isEmpty(longClickMethod))
                            setViewLongClickListener(obj, field,
                                    longClickMethod);
                        ...
                        if (!TextUtils.isEmpty(itemLongClickMethod))
                            setItemLongClickListener(obj, field,
                                    itemLongClickMethod);
                    }else {
                        // 标准ioc注入
                        Inject inject = field.getAnnotation(Inject.class);
     ...
     *
     * @param obj        当前类
     * @param field      当前成员变量或控件
     * @param viewInject 控件id（以InjectView类的成员变量存储）
     */
    public static void indectView(Object obj, Field field, InjectView viewInject) {
    ...
    private static Field[] getPublicFields(Class<?> clazz) {
        ...
        try {
            ...
            Log.log("getDeclaredFields:" + e.toString());
        }
    ...
    private static boolean isCommonClazz(Class<?> clazz) {
        ...
        boolean isok = pkg.startsWith("dhroid")
                || pkg.startsWith(IocContainer.getShare()
                .getApplicationContext().getPackageName());
        if (isok)
            return true;
        if (FWConst.ioc_instal_pkg != null) {
            for (int i = 0; i < FWConst.ioc_instal_pkg.length; i++) {
                if (pkg.startsWith(FWConst.ioc_instal_pkg[i]))
                    return true;
            }
        }
        return false;
